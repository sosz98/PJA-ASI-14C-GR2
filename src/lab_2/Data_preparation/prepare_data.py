"""
Code for data preparation of dataset for further analysis
"""
...
def data_preparation(obesity: pd.DataFrame) -> None:
    ...
    obesity["CAEC"] = pd.Categorical(
        obesity["CAEC"], categories=[0, 1, 2, 3], ordered=True
    )
    obesity["CALC"] = pd.Categorical(
        obesity["CALC"], categories=[0, 1, 2], ordered=True
    )
    obesity["OB_LEVEL"] = pd.Categorical(
        obesity["OB_LEVEL"], categories=[0, 1, 2, 3, 4, 5, 6], ordered=True
    )
